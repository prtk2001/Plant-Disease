...
def resnet_predict(image):
    ...
    with st.spinner('Loading Model...'):
        classifier_model_resnet = tf.keras.models.load_model("models/resnet_model.h5",custom_objects={'KerasLayer':hub.KerasLayer}, compile = False)
    IMAGE_SHAPE = (224, 224)
    image = cv2.resize(image, (IMAGE_SHAPE[0], IMAGE_SHAPE[1]) )
...
def efficient_predict(image):
    ...
    with st.spinner('Loading Model...'):
        classifier_model_efficient = tf.keras.models.load_model("models/efficientv2.h5",custom_objects={'KerasLayer':hub.KerasLayer}, compile = False)
    IMAGE_SHAPE = (224, 224)
    image = cv2.resize(image, (IMAGE_SHAPE[0], IMAGE_SHAPE[1]) )
...
def inception_predict(image):
    ...
    with st.spinner('Loading Model...'):
        classifier_model_inception = tf.keras.models.load_model("models/inception_model.h5",custom_objects={'KerasLayer':hub.KerasLayer}, compile = False)   
    IMAGE_SHAPE = (224, 224)
    image = cv2.resize(image, (IMAGE_SHAPE[0], IMAGE_SHAPE[1]) )
